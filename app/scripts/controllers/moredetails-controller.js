...
angular.module('studymonitorApp')
  .controller('MoredetailsController', function (moredetailsService, feesService, feepaymentdetailsService, $stateParams, $scope, toastr, APP_MESSAGES, $timeout, $http, $window, $cookies, FeeSetup, Calendar, AcademicBatch, $filter, Class, Student) {
    ...
    MoreDetailsCtrl.fetchingStudentAtt = function () {
      async.each(MoreDetailsCtrl.month, function (node, cb) {
          $http({
            "method": "GET",
            "url": 'http://localhost:3000/api/Attendances/reportAttendances?SID=' + MoreDetailsCtrl.schoolCode + '&fromDate=' + node.StartDateFormated + '&toDate=' + node.endDateFormated,
            "headers": {
