...
angular.module('studymonitorApp')
  .controller('AssignmentsController', function (assignmentsService, $scope, $cookies, $timeout, $filter, toastr, APP_MESSAGES, generateexcelFactory, Assignment, Student, $http, Class, Subject, FOsubject) {
    ...
    AssignmentsCtrl.chooseClass = function (classId, date) {
      ...
      for (var i = 0; i < listDateyo.length; i++) {
        ...
        Assignment.find({
          ...
          AssignmentsCtrl.thisweekDates.push(response);
        })
    ...
    AssignmentsCtrl.printData = function () {
      var divToPrint = document.getElementById("assignmentsprintTable");
    ...
    // emails sending start
    AssignmentsCtrl.alertEmails = function (dropData) {
      var fromDate = $filter('date')(dropData.fromDate, 'dd-MM-yyyy')
      var toDate = $filter('date')(dropData.toDate, 'dd-MM-yyyy')
      Student.find({ filter: { where: { schoolId: AssignmentsCtrl.schoolId } } }, function (resp) {
        var emails;
        if (resp) {
          Class.find({ filter: { where: { id: dropData.classId } } }, function (res) {
            dropData.className = res[0].className + " " + res[0].sectionName;
          })
          Subject.find({ filter: { where: { id: dropData.subjectId } } }, function (sub) {
            dropData.subjectName = sub[0].subjectName
          })
          _.each(resp, function (ss) {
            if (emails == undefined) {
              emails = ss.email + ','
            } else {
              emails += ss.email + ','
          ...
          $timeout(function () {
            var sss = dropData.subjectName + '\n' + dropData.title + '\n' + 'for' + '\n' + dropData.className + '\n' + 'from' + '\n' + fromDate + '\n' + 'to' + '\n' + toDate + '<br>' + dropData.description;
            var dd = _.trimEnd(emails, ',');
            var ss1 = dd.split(',');
            if (resp.length == 1) {
              $http({
                "url": 'http://localhost:3000/api/Enquiries/emailNotification',
                "method": "POST",
                "headers": { "Content-Type": "application/json" },
                "data": {
                  "to": dd,
                  "subject": dropData.title,
                  "html": sss,
                }
              }).then(function (response) {
              });
            } else {
              _.forEach(ss1, function (res) {
                $http({
                  "url": 'http://localhost:3000/api/Enquiries/emailNotification',
                  "method": "POST",
                  "headers": { "Content-Type": "application/json" },
    ...
    AssignmentsCtrl.test = function () {
      console.log(AssignmentsCtrl.listOfDates);
      console.log(AssignmentsCtrl.listOdDates);
      console.log(AssignmentsCtrl.thisweekDates);
    }
