...
angular.module('studymonitorApp')
    .controller('BulkuploadController', function ($timeout, $cookies, $rootScope, Student, bulkuploadService, toastr, APP_MESSAGES, Staff, $state, $http, $scope, AcademicBatch, School) {
        ...
        //managerRoleId start
        $http({
            "url": 'http://localhost:3000/api/ManageRoles?filter={"where":{"schoolId":"' + BulkuploadCtrl.schoolId + '","type":"Student"}}',
            "method": "GET",
            "headers": {
        ...
        //managerRoleId start
         $http({
            "url": 'http://localhost:3000/api/ManageRoles?filter={"where":{"schoolId":"' + BulkuploadCtrl.schoolId + '","type":"Staff"}}',
            "method": "GET",
            "headers": {
                "Content-Type": "application/json"
            }
        ...
        BulkuploadCtrl.uploadFiles = function () {
            ...
            var uploadUrl = "http://localhost:3000/api/ImageContainers/StudBulkUpload/upload";
            $http.post(uploadUrl, nd, {
                ...
                .then(function (response) {
                    ...
                    if (response) {
                        BulkuploadCtrl.file = 'http://localhost:3000/api/ImageContainers/StudBulkUpload/download/' + response.data.result[0].filename;
                        ...
                        $http({
                            method: "POST",
                            url: urlBase + "/Students/studentUpload",
                            data: data
                        }).
                        then(function (response) {
        ...
        BulkuploadCtrl.uploadFiles1 = function () {
            ...
            // console.log(fd.get('file').name+fd.get('file').lastModifiedDate);
            // console.log(fd.get('file').name+md1 );
            var uploadUrl = "http://localhost:3000/api/ImageContainers/StaffBulkUpload/upload";
            $http.post(uploadUrl, nd, {
                    transformRequest: angular.identity,
                    headers: {
                        'Content-Type': undefined
                ...
                .then(function (response) {
                    if (response) {
                        BulkuploadCtrl.stafffile = 'http://localhost:3000/api/ImageContainers/StaffBulkUpload/download/' + response.data.result[0].filename;
                        // console.log(BulkuploadCtrl.stafffile);
                        var data = {
                            "staffFile": BulkuploadCtrl.stafffile
                        }
                        // console.log(urlBase + "/Staffs/staffUpload");
                        $http({
                            method: "POST",
                            url: urlBase + "/Staffs/staffUpload",
                            data: data
                        }).
                        then(function (response) {
                            // console.log(response.data);
