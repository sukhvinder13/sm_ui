...
angular.module('studymonitorApp')
    .controller('AttendanceController', function ($cookies, attendanceService, Student, Staff, Attendance, $http, $filter, FOsubject, Subject, $scope, $timeout, School, generateexcelFactory) {
        ...
        AttendanceCtrl.addAttendances = function (flag, type) {
            ...
            $timeout(function () {
                if (abc == true) {
                    ...
                    console.log("slect all deselect");
                    for (var i = 0; i < AttendanceCtrl.dataArray.length; i++) {
                            ...
                            function (response) {
                                if (response.length) {
                                    ...
                                    _.forEach(response, function (res) {
                                        $http({
                                            url: "http://localhost:3000/api/Attendances/" + res.id,
                                            method: 'DELETE',
        ...
        AttendanceCtrl.viewAttendance = function (classId, date, type) {
            ...
            if (type == 'student') {
                ...
                $http({ method: "POST", url: url + "api/Attendances/attendanceDetails", data: data }).
                    then(function (response) {
                        ...
                        for (var i = 0; i < AttendanceCtrl.dataArray.length; i++) {
                            if (AttendanceCtrl.dataArray[i].timeIn) {
                                // console.log("undhi timeIN");
                                // console.log(i);
                                AttendanceCtrl.addLength += 1;
                                // console.log(AttendanceCtrl.addLength);
        ...
        AttendanceCtrl.findDetails = function () {
            ...
            // $http({
            //     "method": "GET",
            //     "url": 'http://localhost:3000/api/Students/getStudents',
