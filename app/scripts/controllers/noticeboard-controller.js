...
angular.module('studymonitorApp')
    .controller('NoticeboardController', function (noticeboardService, $window, $http, $cookies, $timeout, APP_MESSAGES, toastr, generateexcelFactory, $scope, Student, $filter, Noticeboard) {
        ...
        NoticeboardCtrl.uploadFile = function (x) {
            ...
            var uploadUrl = "http://localhost:3000/api/ImageContainers/Notices/upload";
            $http.post(uploadUrl, fd, {
                ...
                .then(function (response) {
                    if (response) {
                        NoticeboardCtrl.file = 'http://localhost:3000/api/ImageContainers/Notices/download/' + response.data.result[0].filename;
                    }
                }, function (error) {
        ...
        NoticeboardCtrl.alertEmails = function (dropData) {
            var date2 = $filter('date')(dropData.date2, 'dd-MM-yyyy');
            var date1 = $filter('date')(dropData.date1, 'dd-MM-yyyy')

            Student.find({ filter: { where: { schoolId: NoticeboardCtrl.schoolId } } }, function (resp) {
                var emails;
                if (resp) {
                    ...
                    // },2000);
                    $timeout(function () {
                        var sss = dropData.title + '\n' + 'are starting from ' + date1 + '\n' + 'to' + '\n' + date2 + '<br>' + dropData.description;
                        var dd = _.trimEnd(emails, ',');
                        var ss1 = dd.split(',');
                        if (resp.length == 1) {
                            $http({
