    ...
    FeeDetailsCtrl.payFee = function (invalid) {
      ...
      _.each(FeeDetailsCtrl.studentDetails.feeSetup, function (setup) {
        _.each(setup.feeitems, function (feeItem) {
          if (feeItem.isSelected) {
            if (feeItem.pay < 20) {
              alert('Payment Amount should be more then Rs.20/-');
              $scope.first = true;
              return;
            }
      ...
      if (FeeDetailsCtrl.paymode === 'onlpay') {
        ...
        feepaymentdetailsService.Pay({ feeItems: feeItems }).then(function (res) {
          if (res) {
            ...
            // $scope.first = true;
            if (FeeDetailsCtrl.smsid !== "") {
              var sender = FeeDetailsCtrl.schoolSmsCode;
            }
            else {
              var sender = 'studym';
            }
            ...
            var msg1 = 'Dear Parents, Fees amount of Rs';
            var amount = FeeDetailsCtrl.paidAmount;
            var contct = FeeDetailsCtrl.studContct;
            var msgc = 'Received on behalf of';
            var studentname = FeeDetailsCtrl.studFulname;
            var schoolName = FeeDetailsCtrl.schoolList[0].schoolName;
            var msg = msg1 + ' ' + amount + ' ' + msgc + ' ' + studentname + ' - ' + schoolName;
            var smsurl = baseurl + uname + '&password=' + pswd + '&to=' + contct + '&from=' + sender + '&text=' + msg;
            var tDate = new Date();
            FeeDetailsCtrl.todayDate = $filter('date')(tDate, "dd-MM-yyyy");
            Smsreport.create({ schoolId: FeeDetailsCtrl.schoolId, studentId: $stateParams.id, smstype: "feeReceipt", date: FeeDetailsCtrl.todayDate, message: msg }, function (response) {
            });
            $http({
              method: 'POST',
              url: smsurl
      ...
      if (payment.discount != 0) {
        ...
        School.find({ filter: { where: { id: stuData.schoolId } } }, function (response) {
          ...
          var printContents = '<!DOCTYPE html><html><head><link rel="stylesheet" type="text/css" href="custom.css" media="all"/><link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css"><style>.right{    border-right-style: none;} .left{border-left-style: none;}</style>' +
            ...
            + '<tr style="border:1px solid black"><th colspan="1" style=""> Class/Section : </th><th colspan="1">' + stuData.class + '-' + stuData.section + '</th><th colspan="1"> Payable Amount:</th><th colspan="2" style="text-align:right;"> ' + payment.payable + '</th></tr>'
