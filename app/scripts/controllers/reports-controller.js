...
angular.module('studymonitorApp')
  .controller('ReportsController', function (School, reportsService, Attendance, FeeSetup, $cookies, Student, FeeCategory, Class, $filter, $scope, $http, Calendar, Staff, Smsreport, FeePayment, ExpensePayment, feesService, feepaymentdetailsService, $stateParams, Deposit, toastr, $timeout) {
    ...
    ReportsCtrl.reportView = function (report) {
      ...
      if (reportValue == "att" && userid == "userstudent") {
        ...
        Student.find({
          ...
          $http({
            "method": "GET",
            "url": 'http://localhost:3000/api/Attendances/reportAttendances?SID=' + ReportsCtrl.schoolCode + '&fromDate=' + ReportsCtrl.date1 + '&toDate=' + ReportsCtrl.date2,
            "headers": {
      ...
      if (reportValue == "att" && userid == "userstaff") {
        ...
        $http({
          "method": "GET",
          "url": 'http://localhost:3000/api/discounts/discountReports?schoolId=' + ReportsCtrl.schoolId + '&classId=' + ReportsCtrl.formFields.classId,
          "headers": {
            "Content-Type": "application/json",
            "Accept": "application/json"
          }
        ...
        // $http({
        //   "method": "GET",
        //   "url": 'http://localhost:3000/api/FeePayments/feeDueReport?schoolId=' + ReportsCtrl.schoolId,
        //   "headers": {
        //     "Content-Type": "application/json",
        //     "Accept": "application/json"
        //   }
    ...
    ReportsCtrl.mailNotify = function () {
      ...
      _.each(enqData[0].classStuDue, function (enqData1) {
        if (enqData1.dueTillNow > 0) {
          ...
          var dates1 = [];
          _.each(enqData1.dateArray, function (dates) {
            var date = $filter('date')(dates, "dd-MM-yyyy");
            dates1.push(date);
            var firstDate = dates1[0];
            var lastDate = dates1[dates1.length - 1];
            var To = 'To';
            ...
            $http({
              "url": 'http://localhost:3000/api/FeePayments/sendSMS?to=' + enqData1.contact + '&text=' + txt,
              "method": "GET",
