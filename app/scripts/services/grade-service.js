...
angular.module('studymonitorApp')
    .service('gradeService', function ($q, Grade) {
        ...
        this.editGrade = function (data) {
            var _activepromise = $q.defer();
            Grade.upsert({id: data.id, gradeName: data.gradeName, gradePoint: data.gradePoint, percentageRangeFrom: data.percentageRangeFrom, percentageRangeTo: data.percentageRangeTo },
                function (response) {
                    _activepromise.resolve(response);
                }, function (error) {
                    _activepromise.reject(error);
